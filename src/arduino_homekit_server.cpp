...
// #define TCP_DEFAULT_KEEPALIVE_COUNT             9    // fault after 9
// failures const int idle = 180; /* 180 sec idle before start sending probes */
...
// max(encrypted_chunk) = 512 + 8(chunk_info) + 18(chacha_info). See
...
#define CLIENT_DEBUG(client, message, ...) \
  DEBUG("[Client %d] " message, client->socket, ##__VA_ARGS__)
#define CLIENT_INFO(client, message, ...) \
  INFO("[Client %d] " message, client->socket, ##__VA_ARGS__)
#define CLIENT_ERROR(client, message, ...) \
  ERROR("[Client %d] " message, client->socket, ##__VA_ARGS__)
...
void server_free(homekit_server_t *server) {
  if (server->pairing_context) pairing_context_free(server->pairing_context);
...
pair_verify_context_t *pair_verify_context_new() {
  pair_verify_context_t *context =
      (pair_verify_context_t *)malloc(sizeof(pair_verify_context_t));
...
void pair_verify_context_free(pair_verify_context_t *context) {
  if (context->secret) free(context->secret);

  if (context->session_key) free(context->session_key);

  if (context->device_public_key) free(context->device_public_key);

  if (context->accessory_public_key) free(context->accessory_public_key);
...
void client_context_free(client_context_t *c) {
  if (c->verify_context) pair_verify_context_free(c->verify_context);
  ...
  if (c->endpoint_params) query_params_free(c->endpoint_params);
...
pairing_context_t *pairing_context_new() {
  pairing_context_t *context =
      (pairing_context_t *)malloc(sizeof(pairing_context_t));
...
void client_notify_characteristic(homekit_characteristic_t *ch,
                                  homekit_value_t value, void *client);
...
void write_characteristic_json(json_stream *json, client_context_t *client,
  ...
  if (format & characteristic_format_perms) {
    ...
    if (ch->permissions & homekit_permissions_paired_write)
      json_string(json, "pw");
    if (ch->permissions & homekit_permissions_notify) json_string(json, "ev");
    if (ch->permissions & homekit_permissions_additional_authorization)
      json_string(json, "aa");
    if (ch->permissions & homekit_permissions_timed_write)
      json_string(json, "tw");
    if (ch->permissions & homekit_permissions_hidden) json_string(json, "hd");
    json_array_end(json);
  }

  if ((format & characteristic_format_events) &&
      (ch->permissions & homekit_permissions_notify)) {
    bool events = homekit_characteristic_has_notify_callback(
        ch, client_notify_characteristic, client);
    json_string(json, "ev");
  ...
  if (ch->permissions & homekit_permissions_paired_read) {
    homekit_value_t v = value           ? *value
                        : ch->getter_ex ? ch->getter_ex(ch)
                                        : ch->value;
    ...
    if (v.is_null) {
      ...
      ERROR(
          "Characteristic value format is different from characteristic "
          "format");
    } else {
      switch (v.format) {
        case homekit_format_bool: {
        ...
        case homekit_format_data: {
          ...
          if (!v.data_value || v.data_size == 0) {
            ...
            size_t encoded_data_size =
                base64_encoded_size(v.data_value, v.data_size);
...
void write(client_context_t *context, byte *data, int data_size) {
  ...
  if (write_size != data_size) {
    CLIENT_ERROR(context, "socket.write, data_size=%d, write_size=%d",
                 data_size, write_size);
    context->error_write = true;
    // Error write when :
    // 1. remote client is disconnected
    // 2. data_size is larger than the tcp internal send buffer
    // But We has limited the data_size to 538, and TCP_SND_BUF = 1072. (See the
    // comments on HOMEKIT_JSONBUFFER_SIZE) So we believe here is disconnected.
    context->disconnect = true;
    homekit_server_close_client(context->server, context);
    // We consider the socket is 'closed' when error in writing (eg. the remote
    // client is disconnected, NO tcp ack receive). Closing the socket causes
    // memory-leak if some data has not been sent (the write_buffer did not
    // free) To fix this memory-leak, add tcp_abandon(_pcb, 0); in
    // ClientContext.h of ESP8266WiFi-library.
  }
}

int client_send_encrypted_(client_context_t *context, byte *payload,
                           size_t size) {
  CLIENT_DEBUG(context, "Send encrypted of size %d", size);
  // max(size) = HOMEKIT_JSONBUFFER_SIZE + chunk_info(8) = 512 + 8 = 520
  if (!context || !context->encrypted) return -1;
  ...
  while (payload_offset < size) {
    ...
    int r = crypto_chacha20poly1305_encrypt(
        context->read_key, nonce, aead, 2, payload + payload_offset, chunk_size,
...
int client_decrypt_(client_context_t *context, byte *payload,
                    size_t payload_size, byte *decrypted,
  ...
  *decrypted_size = required_decrypted_size;
  ...
  while (payload_offset < payload_size) {
    size_t chunk_size =
        payload[payload_offset] + payload[payload_offset + 1] * 256;
    ...
    int r = crypto_chacha20poly1305_decrypt(
        ...
        payload + payload_offset + 2, chunk_size + 16, decrypted,
        &decrypted_len);
    if (r) {
...
void client_notify_characteristic(homekit_characteristic_t *ch,
                                  homekit_value_t value, void *context) {
  ...
  // DEBUG("Got characteristic %d.%d change event", ch->service->accessory->id,
  // ch->id);
  ...
  characteristic_event_t *event =
      (characteristic_event_t *)malloc(sizeof(characteristic_event_t));
...
void client_send(client_context_t *context, byte *data, size_t data_size) {
  CLIENT_DEBUG(context, "send data size=%d, encrypted=%s", data_size,
               context->encrypted ? "true" : "false");
...
void send_client_events(client_context_t *context, client_event_t *events) {
  ...
  static const char PROGMEM http_headers[] =
      "EVENT/1.0 200 OK\r\n"
  ...
  json_stream *json =
      json_new(HOMEKIT_JSONBUFFER_SIZE, client_send_chunk, context);
  ...
  while (e) {
    json_object_start(json);
    write_characteristic_json(json, context, e->characteristic,
                              (characteristic_format_t)0, &e->value);
...
void send_tlv_error_response(client_context_t *context, int state,
                             TLVError error) {
...
void send_tlv_response(client_context_t *context, tlv_values_t *values) {
  ...
  static const char PROGMEM http_headers_pgm[] =
      "HTTP/1.1 200 OK\r\n"
  ...
  int response_len =
      snprintf(response, response_size, http_headers, payload_size);

  if (response_size - response_len < payload_size + 1) {
    CLIENT_ERROR(
        context,
        "Incorrect response buffer size %d: headers took %d, payload size %d",
...
static const char PROGMEM json_200_response_headers_progmem[] =
    "HTTP/1.1 200 OK\r\n"
    "Content-Type: application/hap+json\r\n"
    "Transfer-Encoding: chunked\r\n"
    "Connection: keep-alive\r\n\r\n";

static const char PROGMEM json_207_response_headers_progmem[] =
    "HTTP/1.1 207 Multi-Status\r\n"
    "Content-Type: application/hap+json\r\n"
    "Transfer-Encoding: chunked\r\n"
    "Connection: keep-alive\r\n\r\n";

void send_json_response(client_context_t *context, int status_code,
  ...
  int response_size =
      strlen(http_headers) + payload_size + strlen(status_text) + 32;
  char *response = (char *)malloc(response_size);
  if (!response) {
    CLIENT_ERROR(context, "Failed to allocate response buffer of size %d",
                 response_size);
    return;
  }
  int response_len = snprintf(response, response_size, http_headers,
                              status_code, status_text, payload_size);
...
void send_json_error_response(client_context_t *context, int status_code,
                              HAPStatus status) {
  byte buffer[32];
  int size =
      snprintf((char *)buffer, sizeof(buffer), "{\"status\": %d}", status);
...
bool homekit_client_is_admin() {
  if (!current_client_context) return false;
...
int homekit_client_send(unsigned char *data, size_t size) {
  if (!current_client_context) return -1;
...
void homekit_server_on_identify(client_context_t *context) {
  ...
  homekit_characteristic_t *ch_identify =
      homekit_service_characteristic_by_type(accessory_info,
...
void homekit_server_on_pair_setup(client_context_t *context, const byte *data,
                                  size_t size) {
  DEBUG("Pair Setup");
  DEBUG_HEAP();
  DEBUG_TIME_BEGIN();
  ...
  switch (tlv_get_integer_value(message, TLVType_State, -1)) {
    case 1: {
      ...
      if (context->server->pairing_context) {
        ...
        context->server->pairing_context =
            saved_preinit_pairing_context;  /// pairing_context_new();
        context->server->pairing_context->client = context;
      }

      int r = 0;
      size_t salt_size = 0;
      crypto_srp_get_salt(context->server->pairing_context->srp, NULL,
                          &salt_size);
      byte *salt = (byte *)malloc(salt_size);
      r = crypto_srp_get_salt(context->server->pairing_context->srp, salt,
                              &salt_size);
      if (r) {
    ...
    case 3: {
      ...
      int r = crypto_srp_compute_key(
          context->server->pairing_context->srp, device_public_key->value,
          device_public_key->size, context->server->pairing_context->public_key,
          context->server->pairing_context->public_key_size);
      ...
      if (r) {
        CLIENT_ERROR(context, "Failed to compute SRP shared secret (code %d)",
                     r);
        send_tlv_error_response(context, 4, TLVError_Authentication);
        break;
      }
      ...
      // watchdog_check_begin();
      r = crypto_srp_verify(context->server->pairing_context->srp, proof->value,
                            proof->size);
      ...
      crypto_srp_get_proof(context->server->pairing_context->srp, NULL,
                           &server_proof_size);

      byte *server_proof = (byte *)malloc(server_proof_size);
      // watchdog_check_begin();
      r = crypto_srp_get_proof(context->server->pairing_context->srp,
                               server_proof, &server_proof_size);
    ...
    case 5: {
      ...
      r = crypto_srp_hkdf(context->server->pairing_context->srp, (byte *)salt1,
                          sizeof(salt1) - 1, (byte *)info1, sizeof(info1) - 1,
      ...
      crypto_chacha20poly1305_decrypt(
          ...
          tlv_encrypted_data->value, tlv_encrypted_data->size, NULL,
          &decrypted_data_size);
      ...
      tlv_t *tlv_device_id =
          tlv_get_value(decrypted_message, TLVType_Identifier);
      if (!tlv_device_id) {
        CLIENT_ERROR(context,
                     "Invalid encrypted payload: no device identifier");
      ...
      tlv_t *tlv_device_public_key =
          tlv_get_value(decrypted_message, TLVType_PublicKey);
      if (!tlv_device_public_key) {
        CLIENT_ERROR(context,
                     "Invalid encrypted payload: no device public key");
      ...
      tlv_t *tlv_device_signature =
          tlv_get_value(decrypted_message, TLVType_Signature);
      ...
      r = crypto_ed25519_import_public_key(&device_key,
                                           tlv_device_public_key->value,
                                           tlv_device_public_key->size);
      if (r) {
        CLIENT_ERROR(context, "Failed to import device public Key (code %d)",
                     r);
      ...
      r = crypto_srp_hkdf(context->server->pairing_context->srp, (byte *)salt2,
                          sizeof(salt2) - 1, (byte *)info2, sizeof(info2) - 1,
      ...
      size_t device_info_size =
          device_x_size + tlv_device_id->size + tlv_device_public_key->size;
      byte *device_info = (byte *)malloc(device_info_size);
      memcpy(device_info, device_x, device_x_size);
      memcpy(device_info + device_x_size, tlv_device_id->value,
             tlv_device_id->size);
      ...
      r = crypto_ed25519_verify(&device_key, device_info, device_info_size,
                                tlv_device_signature->value,
                                tlv_device_signature->size);
      if (r) {
        CLIENT_ERROR(context, "Failed to generate DeviceX (code %d)", r);
      ...
      r = homekit_storage_add_pairing((const char *)tlv_device_id->value,
                                      &device_key, pairing_permissions_admin);
      if (r) {
      ...
      char *device_id =
          strndup((const char *)tlv_device_id->value, tlv_device_id->size);
      ...
      r = crypto_ed25519_export_public_key(&context->server->accessory_key,
                                           accessory_public_key,
                                           &accessory_public_key_size);
      if (r) {
        CLIENT_ERROR(context, "Failed to export accessory public key (code %d)",
                     r);
      ...
      size_t accessory_info_size =
          HKDF_HASH_SIZE + accessory_id_size + accessory_public_key_size;
      ...
      r = crypto_srp_hkdf(context->server->pairing_context->srp, (byte *)salt3,
                          sizeof(salt3) - 1, (byte *)info3, sizeof(info3) - 1,
      ...
      memcpy(accessory_info + accessory_x_size, context->server->accessory_id,
             accessory_id_size);
      ...
      crypto_ed25519_sign(&context->server->accessory_key, accessory_info,
                          accessory_info_size, NULL, &accessory_signature_size);
      ...
      r = crypto_ed25519_sign(&context->server->accessory_key, accessory_info,
                              accessory_info_size, accessory_signature,
                              &accessory_signature_size);

      if (r) {
        CLIENT_ERROR(context,
                     "Failed to generate accessory signature (code %d)", r);
      ...
      crypto_chacha20poly1305_encrypt(
          shared_secret, (byte *)"\x0\x0\x0\x0PS-Msg06", NULL, 0, response_data,
          response_data_size, NULL, &encrypted_response_data_size);

      byte *encrypted_response_data =
          (byte *)malloc(encrypted_response_data_size);
      r = crypto_chacha20poly1305_encrypt(
          shared_secret, (byte *)"\x0\x0\x0\x0PS-Msg06", NULL, 0, response_data,
          response_data_size, encrypted_response_data,
...
void homekit_server_on_pair_verify(client_context_t *context, const byte *data,
                                   size_t size) {
  ...
  switch (tlv_get_integer_value(message, TLVType_State, -1)) {
    case 1: {
      ...
      if (r) {
        CLIENT_ERROR(
            context,
            "Failed to initialize device Curve25519 public key (code %d)", r);
        send_tlv_error_response(context, 2, TLVError_Unknown);
        break;
      }

      r = crypto_curve25519_import_public(&device_key,
                                          tlv_device_public_key->value,
                                          tlv_device_public_key->size);
      if (r) {
        CLIENT_ERROR(context,
                     "Failed to import device Curve25519 public key (code %d)",
                     r);
      ...
      if (r) {
        CLIENT_ERROR(context,
                     "Failed to generate accessory Curve25519 key (code %d)",
                     r);
        crypto_curve25519_done(&device_key);
        send_tlv_error_response(context, 2, TLVError_Unknown);
        break;
      ...
      r = crypto_curve25519_export_public(&my_key, my_key_public,
                                          &my_key_public_size);
      if (r) {
        CLIENT_ERROR(
            context,
            "Failed to export accessory Curve25519 public key (code %d)", r);
        free(my_key_public);
      ...
      crypto_curve25519_shared_secret(&my_key, &device_key, NULL,
                                      &shared_secret_size);
      ...
      if (r) {
        CLIENT_ERROR(context,
                     "Failed to generate Curve25519 shared secret (code %d)",
                     r);
        free(shared_secret);
        free(my_key_public);
        send_tlv_error_response(context, 2, TLVError_Unknown);
      ...
      size_t accessory_id_size = sizeof(context->server->accessory_id) - 1;
      size_t accessory_info_size =
          my_key_public_size + accessory_id_size + tlv_device_public_key->size;
      ...
      tlv_add_value(sub_response, TLVType_Identifier,
                    (const byte *)context->server->accessory_id,
                    accessory_id_size);
      ...
      crypto_hkdf(shared_secret, shared_secret_size, salt, sizeof(salt) - 1,
                  info, sizeof(info) - 1, NULL, &session_key_size);

      byte *session_key = (byte *)malloc(session_key_size);
      r = crypto_hkdf(shared_secret, shared_secret_size, salt, sizeof(salt) - 1,
                      info, sizeof(info) - 1, session_key, &session_key_size);
      if (r) {
      ...
      CLIENT_DEBUG(context, "Encrypting response");
      size_t encrypted_response_data_size = 0;
      crypto_chacha20poly1305_encrypt(session_key,
                                      (byte *)"\x0\x0\x0\x0PV-Msg02", NULL, 0,
      ...
      if (r) {
        CLIENT_ERROR(context, "Failed to encrypt sub response data (code %d)",
                     r);
        free(encrypted_response_data);
        free(session_key);
        free(shared_secret);
      ...
      tlv_values_t *response = tlv_new();
      tlv_add_integer_value(response, TLVType_State, 1, 2);
      tlv_add_value(response, TLVType_PublicKey, my_key_public,
                    my_key_public_size);
      tlv_add_value(response, TLVType_EncryptedData, encrypted_response_data,
                    encrypted_response_data_size);
      ...
      context->verify_context->device_public_key =
          (byte *)malloc(tlv_device_public_key->size);
    ...
    case 3: {
      ...
      size_t decrypted_data_size = 0;
      crypto_chacha20poly1305_decrypt(
          context->verify_context->session_key, (byte *)"\x0\x0\x0\x0PV-Msg03",
          NULL, 0, tlv_encrypted_data->value, tlv_encrypted_data->size, NULL,
          &decrypted_data_size);

      byte *decrypted_data = (byte *)malloc(decrypted_data_size);
      r = crypto_chacha20poly1305_decrypt(
          context->verify_context->session_key, (byte *)"\x0\x0\x0\x0PV-Msg03",
          NULL, 0, tlv_encrypted_data->value, tlv_encrypted_data->size,
      ...
      CLIENT_DEBUG(context, "Verifying device signature");
      r = crypto_ed25519_verify(&pairing.device_key, device_info,
                                device_info_size, tlv_device_signature->value,
                                tlv_device_signature->size);
      free(device_info);
      tlv_free(decrypted_message);
      ...
      r = crypto_hkdf(context->verify_context->secret,
                      context->verify_context->secret_size, salt,
                      sizeof(salt) - 1, read_info, sizeof(read_info) - 1,
                      context->read_key, &read_key_size);
      if (r) {
        CLIENT_ERROR(context, "Failed to derive read encryption key (code %d)",
                     r);
        pair_verify_context_free(context->verify_context);
        context->verify_context = NULL;
      ...
      r = crypto_hkdf(context->verify_context->secret,
                      context->verify_context->secret_size, salt,
                      sizeof(salt) - 1, write_info, sizeof(write_info) - 1,
      ...
      if (r) {
        CLIENT_ERROR(context, "Failed to derive write encryption key (code %d)",
                     r);
        send_tlv_error_response(context, 4, TLVError_Unknown);
        break;
      }
      ...
      CLIENT_INFO(context,
                  "Verification successful, secure session established");
...
void homekit_server_on_get_accessories(client_context_t *context) {
  ...
  for (homekit_accessory_t **accessory_it =
    ...
    for (homekit_service_t **service_it = accessory->services; *service_it;
         service_it++) {
      ...
      for (homekit_characteristic_t **ch_it = service->characteristics; *ch_it;
           ch_it++) {
...
void write_characteristic_error(json_stream *json, uint32_t aid, uint32_t iid,
                                int status) {
  json_object_start(json);
  json_string(json, "aid");
...
void homekit_server_on_get_characteristics(client_context_t *context) {
  ...
  if (!id_param) {
    CLIENT_ERROR(context,
                 "Invalid get characteristics request: missing ID parameter");
...
HAPStatus process_characteristics_update(const cJSON *j_ch,
  ...
  if (j_aid->type != cJSON_Number) {
    CLIENT_ERROR(context,
                 "Failed to process request: \"aid\" field is not a number");
    return HAPStatus_NoResource;
  }
  ...
  if (j_iid->type != cJSON_Number) {
    CLIENT_ERROR(context,
                 "Failed to process request: \"iid\" field is not a number");
    return HAPStatus_NoResource;
  }
  ...
  if (!ch) {
    CLIENT_ERROR(context,
                 "Failed to process request to update %u.%u: "
  ...
  if (j_value) {
    ...
    if (!(ch->permissions & homekit_permissions_paired_write)) {
      CLIENT_ERROR(context, "Failed to update %d.%d: no write permission", aid,
                   iid);
    ...
    switch (ch->format) {
      case homekit_format_bool: {
        ...
        if (j_value->type == cJSON_True) {
          ...
          CLIENT_ERROR(context,
                       "Failed to update %d.%d: value is not a boolean or 0/1",
                       aid, iid);
          return HAPStatus_InvalidValue;
        }

        CLIENT_DEBUG(context, "Updating characteristic %d.%d with boolean %s",
                     aid, iid, value ? "true" : "false");
      ...
      case homekit_format_uint64:
      case homekit_format_int: {
        // We accept boolean values here in order to fix a bug in HomeKit.
        // HomeKit sometimes sends a boolean instead of an integer of value 0
        // or 1.
        if (j_value->type != cJSON_Number && j_value->type != cJSON_False &&
            j_value->type != cJSON_True) {
          CLIENT_ERROR(context, "Failed to update %d.%d: value is not a number",
                       aid, iid);
          return HAPStatus_InvalidValue;
        }
        ...
        if (ch->min_value) min_value = *ch->min_value;
        if (ch->max_value) max_value = *ch->max_value;

        double value = j_value->valuedouble;
        if (value < min_value || value > max_value) {
          CLIENT_ERROR(
              context,
              "Failed to update %d.%d: value %g is not in range %g..%g", aid,
              iid, value, min_value, max_value);
          return HAPStatus_InvalidValue;
        }
        ...
        if (ch->valid_values.count) {
          ...
          if (!matches) {
            CLIENT_ERROR(
                context,
                "Failed to update %d.%d: value is not one of valid values", aid,
                iid);
            return HAPStatus_InvalidValue;
          }
        }
        ...
        if (ch->valid_values_ranges.count) {
          ...
          if (!matches) {
            CLIENT_ERROR(
                context,
                "Failed to update %d.%d: value is not in valid values range",
                aid, iid);
            return HAPStatus_InvalidValue;
          }
        }

        CLIENT_DEBUG(context, "Updating characteristic %d.%d with integer %g",
                     aid, iid, value);
        ...
        switch (ch->format) {
          ...
          default:
            CLIENT_ERROR(context,
                         "Unexpected format when updating numeric value: %d",
                         ch->format);
            return HAPStatus_InvalidValue;
        }
      ...
      case homekit_format_float: {
        ...
        if ((ch->min_value && value < *ch->min_value) ||
          ...
          CLIENT_ERROR(context, "Failed to update %d.%d: value is not in range",
                       aid, iid);
          return HAPStatus_InvalidValue;
        }

        CLIENT_DEBUG(context, "Updating characteristic %d.%d with %g", aid, iid,
                     value);
      ...
      case homekit_format_string: {
        if (j_value->type != cJSON_String) {
          CLIENT_ERROR(context, "Failed to update %d.%d: value is not a string",
                       aid, iid);
          return HAPStatus_InvalidValue;
        }
        ...
        if (strlen(value) > max_len) {
          CLIENT_ERROR(context, "Failed to update %d.%d: value is too long",
                       aid, iid);
          return HAPStatus_InvalidValue;
        }

        CLIENT_DEBUG(context, "Updating characteristic %d.%d with \"%s\"", aid,
                     iid, value);
      ...
      case homekit_format_tlv: {
        ...
        size_t tlv_size =
            base64_decoded_size((unsigned char *)value, value_len);
        byte *tlv_data = (byte *)malloc(tlv_size);
        if (base64_decode_((byte *)value, value_len, tlv_data) < 0) {
          free(tlv_data);
          CLIENT_ERROR(context, "Failed to update %d.%d: error Base64 decoding",
                       aid, iid);
          return HAPStatus_InvalidValue;
        }
        ...
        if (r) {
          CLIENT_ERROR(context, "Failed to update %d.%d: error parsing TLV",
                       aid, iid);
          return HAPStatus_InvalidValue;
        }

        CLIENT_DEBUG(context, "Updating characteristic %d.%d with TLV:", aid,
                     iid);
        for (tlv_t *t = tlv_values->head; t; t = t->next) {
          char *escaped_payload = binary_to_string(t->value, t->size);
          CLIENT_DEBUG(context, "  Type %d value (%d bytes): %s", t->type,
                       t->size, escaped_payload);
          free(escaped_payload);
        }
      ...
      case homekit_format_data: {
        ...
        CLIENT_DEBUG(context, "Updating characteristic %d.%d with Data:", aid,
                     iid);
  ...
  if (j_events) {
    ...
    if (j_events->type == cJSON_True) {
      homekit_characteristic_add_notify_callback(
          ch, client_notify_characteristic, context);
    } else {
...
void homekit_server_on_update_characteristics(client_context_t *context,
  ...
  if (!characteristics) {
    CLIENT_ERROR(context,
                 "Failed to parse request: no \"characteristics\" field");
    cJSON_Delete(json);
    send_json_error_response(context, 400, HAPStatus_InvalidValue);
    return;
  }
  if (characteristics->type != cJSON_Array) {
    CLIENT_ERROR(
        context,
        "Failed to parse request: \"characteristics\" field is not an list");
    cJSON_Delete(json);
    send_json_error_response(context, 400, HAPStatus_InvalidValue);
    return;
  }
  ...
  for (int i = 0; i < cJSON_GetArraySize(characteristics); i++) {
    ...
    if (statuses[i] != HAPStatus_Success) has_errors = true;
  }

  if (!has_errors) {
    CLIENT_DEBUG(
        context,
        "There were no processing errors, sending No Content response");

    send_204_response(context);
  } else {
    CLIENT_DEBUG(context,
                 "There were processing errors, sending Multi-Status response");
    client_send_P(context, json_207_response_headers_progmem);
    ...
    for (int i = 0; i < cJSON_GetArraySize(characteristics); i++) {
      ...
      json_uint32(
          json1, cJSON_GetObjectItem(j_ch, "aid")->valuedouble);  //->valueint);
      json_string(json1, "iid");
      json_uint32(
          json1, cJSON_GetObjectItem(j_ch, "iid")->valuedouble);  //->valueint);
...
void homekit_server_on_pairings(client_context_t *context, const byte *data,
                                size_t size) {
  ...
  switch (tlv_get_integer_value(message, TLVType_Method, -1)) {
    case TLVMethod_AddPairing: {
      ...
      if (!(context->permissions & pairing_permissions_admin)) {
        CLIENT_ERROR(context,
                     "Refusing to add pairing to non-admin controller");
      ...
      if (!tlv_device_identifier) {
        CLIENT_ERROR(context,
                     "Invalid add pairing request: no device identifier");
        send_tlv_error_response(context, 2, TLVError_Unknown);
        break;
      }
      tlv_t *tlv_device_public_key = tlv_get_value(message, TLVType_PublicKey);
      if (!tlv_device_public_key) {
        CLIENT_ERROR(context,
                     "Invalid add pairing request: no device public key");
        send_tlv_error_response(context, 2, TLVError_Unknown);
        break;
      }
      int device_permissions =
          tlv_get_integer_value(message, TLVType_Permissions, -1);
      if (device_permissions == -1) {
        CLIENT_ERROR(context,
                     "Invalid add pairing request: no device permissions");
        send_tlv_error_response(context, 2, TLVError_Unknown);
        break;
      }
      ...
      if (!homekit_storage_find_pairing(device_identifier, &pairing)) {
        ...
        if (pairing_public_key_size != tlv_device_public_key->size ||
            memcmp(tlv_device_public_key->value, pairing_public_key,
                   pairing_public_key_size)) {
          CLIENT_ERROR(context,
                       "Failed to add pairing: pairing public key differs from "
                       "given one");
          free(pairing_public_key);
          free(device_identifier);
          send_tlv_error_response(context, 2, TLVError_Unknown);
        ...
        r = homekit_storage_update_pairing(device_identifier,
                                           device_permissions);
        if (r) {
          CLIENT_ERROR(context,
                       "Failed to add pairing: storage error (code %d)", r);
          free(device_identifier);
          send_tlv_error_response(context, 2, TLVError_Unknown);
          break;
        }
        ...
        r = homekit_storage_add_pairing(device_identifier, &device_key,
                                        device_permissions);
        if (r) {
          CLIENT_ERROR(context,
                       "Failed to add pairing: storage error (code %d)", r);
    ...
    case TLVMethod_RemovePairing: {
      ...
      if (!(context->permissions & pairing_permissions_admin)) {
        CLIENT_ERROR(context,
                     "Refusing to remove pairing to non-admin controller");
        send_tlv_error_response(context, 2, TLVError_Authentication);
        break;
      }
      ...
      if (!tlv_device_identifier) {
        CLIENT_ERROR(context,
                     "Invalid remove pairing request: no device identifier");
        send_tlv_error_response(context, 2, TLVError_Unknown);
        break;
      }
      ...
      if (!homekit_storage_find_pairing(device_identifier, &pairing)) {
        ...
        if (r) {
          CLIENT_ERROR(context,
                       "Failed to remove pairing: storage error (code %d)", r);
          free(device_identifier);
          send_tlv_error_response(context, 2, TLVError_Unknown);
          break;
        }
        ...
        while (c) {
          if (c->pairing_id == pairing.id) c->disconnect = true;
          c = c->next;
        }
    ...
    case TLVMethod_ListPairings: {
      ...
      if (!(context->permissions & pairing_permissions_admin)) {
        CLIENT_INFO(context,
                    "Refusing to list pairings to non-admin controller");
        send_tlv_error_response(context, 2, TLVError_Authentication);
        break;
      }
      ...
      while (!homekit_storage_next_pairing(&it, &pairing)) {
        ...
        r = crypto_ed25519_export_public_key(&pairing.device_key, public_key,
                                             &public_key_size);
        ...
        tlv_add_integer_value(response, TLVType_Permissions, 1,
                              pairing.permissions);
...
int homekit_server_on_url(http_parser *parser, const char *data,
                          size_t length) {
...
int homekit_server_on_body(http_parser *parser, const char *data,
                           size_t length) {
  DEBUG("http_parser lenght=%d", length);
  client_context_t *context = (client_context_t *)parser->data;
  context->body =
      (char *)realloc(context->body, context->body_length + length + 1);
    ...
    switch (context->endpoint) {
      ...
      case HOMEKIT_ENDPOINT_UPDATE_CHARACTERISTICS: {
        homekit_server_on_update_characteristics(
            context, (const byte *)context->body, context->body_length);
        break;
      }
      case HOMEKIT_ENDPOINT_PAIRINGS: {
        homekit_server_on_pairings(context, (const byte *)context->body,
                                   context->body_length);
        break;
      }
...
void homekit_client_process(client_context_t *context) {
  ...
  if (available_len > 0) {
    ...
    // read or readBytes
    data_len =
        context->socket->read(context->data + context->data_available, size);
  }
  if (data_len == 0) {
  ...
  CLIENT_DEBUG(context, "Got %d incomming data, encrypted is %s", data_len,
               context->encrypted ? "true" : "false");
  ...
  if (context->encrypted) {
    ...
    int r = client_decrypt_(context, context->data, data_len, decrypted,
                            &decrypted_size);
    ...
    CLIENT_DEBUG(context, "Decrypted %d bytes, available %d", decrypted_size,
                 context->data_available);

    payload = decrypted;
    payload_size = decrypted_size;
    if (payload_size) print_binary("Decrypted data", payload, payload_size);
  } else {
...
void homekit_server_close_client(homekit_server_t *server,
  ...
  if (context->server->clients == context) {
    ...
    client_context_t *c = context->server->clients;
    while (c->next && c->next != context) c = c->next;
    if (c->next) c->next = c->next->next;
  }

  homekit_accessories_clear_notify_callbacks(
      context->server->config->accessories, client_notify_characteristic,
      context);
...
void homekit_server_process_notifications(homekit_server_t *server) {
  ...
  while (context) {
    ...
    if (context->event_queue && q_pop(context->event_queue, &event)) {
      ...
      client_event_t *events_head =
          (client_event_t *)malloc(sizeof(client_event_t));
...
void homekit_server_process(homekit_server_t *server) {
  ...
  client_context_t *context = server->clients;
  while (context) {
    // homekit_client_process includes {handle data and stop disconnected
    // client}
    //		do{
    //			if(homekit_client_need_process_data(context)){
    //				CLIENT_INFO(context, "Step is %d",
    // context->step);
    //			}
    //			delay(10);
...
//=====================================================
// Arduino ESP8266 MDNS: call this funciton only once when WiFi STA is
// connected!
//=====================================================
...
void homekit_mdns_init(homekit_server_t *server) {
  ...
  homekit_characteristic_t *name = homekit_service_characteristic_by_type(
      accessory_info, HOMEKIT_CHARACTERISTIC_NAME);
  if (!name) {
    ERROR(
        "Invalid accessory declaration: "
        "no Name characteristic in AccessoryInfo service");
    return;
  }

  homekit_characteristic_t *model = homekit_service_characteristic_by_type(
      accessory_info, HOMEKIT_CHARACTERISTIC_MODEL);
  if (!model) {
    ERROR(
        "Invalid accessory declaration: "
  ...
  MDNSResponder::hMDNSService mdns_service =
      MDNS.addService(name->value.string_value, HOMEKIT_MDNS_SERVICE,
                      HOMEKIT_MDNS_PROTO, HOMEKIT_SERVER_PORT);
  // Set a service specific callback for dynamic service TXT items.
  // The callback is called, whenever service TXT items are needed for the given
  // service.
  MDNS.setDynamicServiceTxtCallback(
      mdns_service, [](const MDNSResponder::hMDNSService p_hService) {
  ...
  // cycles, etc. MDNS.addServiceTxt(mdns_service, "c#",
  ...
  // MDNS.addServiceTxt(HAP_SERVICE, HOMEKIT_MDNS_PROTO, "sf", (server->paired)
  // ? "0" : "1");
  MDNS.addServiceTxt(mdns_service, "ci",
  ...
  if (server->config->setupId) {
    ...
    size_t data_size =
        strlen(server->config->setupId) + strlen(server->accessory_id) + 1;
    char *data = (char *)malloc(data_size);
    snprintf(data, data_size, "%s%s", server->config->setupId,
             server->accessory_id);
    data[data_size - 1] = 0;
  ...
  //		" and \"I Don't Have a Code\". \nThis Accessory will show on
  // your iOS device.");
}

// Used to update the config_number ("c#" value of Bonjour)
// Call this function when an accessory, service, or characteristic is added or
// removed on the accessory server. See the official HAP specification for more
// information.
...
void homekit_server_init(homekit_server_config_t *config) {
  if (!config->accessories) {
    ERROR(
        "Error initializing HomeKit accessory server: "
  ...
  if (config->password) {
    ...
    if (strlen(p) != 10 ||
        !(ISDIGIT(p[0]) && ISDIGIT(p[1]) && ISDIGIT(p[2]) && p[3] == '-' &&
          ISDIGIT(p[4]) && ISDIGIT(p[5]) && p[6] == '-' && ISDIGIT(p[7]) &&
  ...
  if (config->setupId) {
    ...
    if (strlen(p) != 4 || !(ISBASE36(p[0]) && ISBASE36(p[1]) &&
                            ISBASE36(p[2]) && ISBASE36(p[3]))) {
      ERROR(
          "Error initializing HomeKit accessory server: "
  ...
  if (r == 0) {
    ...
    if (!r) r = homekit_storage_load_accessory_key(&server->accessory_key);
  }
  ...
  while (!homekit_storage_next_pairing(&pairing_it, &pairing)) {
    if (pairing.permissions & pairing_permissions_admin) {
      INFO("Found admin pairing with %s, disabling pair setup",
           pairing.device_id);
      server->paired = true;
      break;
    }
...
void homekit_server_reset() { homekit_storage_reset(); }
...
int homekit_get_accessory_id(char *buffer, size_t size) {
  if (size < ACCESSORY_ID_SIZE + 1) return -1;
...
bool arduino_homekit_preinit(homekit_server_t *server) {
  ...
  preinit_pairing_context->public_key =
      (byte *)malloc(preinit_pairing_context->public_key_size);
...
void arduino_homekit_setup(homekit_server_config_t *config) {
  ...
  arduino_homekit_gotiphandler =
      WiFi.onStationModeGotIP([](WiFiEventStationModeGotIP gotip) {
